...
# 第三步：修改现有 ENI 的安全组集合。
#
# 这个 resource 块看起来没有写 ENI ID，是因为目标 ID 保存在 Terraform state 中：
#   volcenginecc_vpc_eni.target -> eni-xxxxxxxx
#
# 第一次使用当前模板时执行一次 terraform import，建立上述绑定。
# 后续 plan/apply 会复用持久化 state，并对该 ENI 执行原地更新。
# 如果没有 import 就直接 apply，Terraform 会把它当成待创建的新 ENI，不能执行。
resource "volcenginecc_vpc_eni" "target" {
  ...
  lifecycle {
    # 即使误执行 terraform destroy，也禁止 Terraform 删除用户原有 ENI。
